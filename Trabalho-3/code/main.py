...
#         plt.plot(dados["posicoes"], dados["densidadeFinal"], label=f"{chave}")
#     plt.plot(posicaoEspacial, condicaoInicial, 'k--', label="Cond. Inicial (Analítica)")
#     plt.title(f'Estabilidade CFL ({nomeMetodo})')
#     plt.xlabel('Posição Espacial')
...
#     plt.plot(posicaoEspacial, resultados[nomeMetodo]["t=1"], label=f'{nomeMetodo} t=1')
...
#     plt.subplot(2, 2, 2)
#     plt.plot(posicaoEspacial, condicaoInicial, label='Cond. Inicial', linestyle='--')
#     plt.plot(posicaoEspacial, resultados[nomeMetodo]["t=3"], label=f'{nomeMetodo} t=3')
...
#     plt.subplot(2, 2, 3)
#     plt.plot(posicaoEspacial, condicaoInicial, label='Cond. Inicial', linestyle='--')
#     plt.plot(posicaoEspacial, resultados[nomeMetodo]["t=5"], label=f'{nomeMetodo} t=5')
...
#     # Criando tabelas LaTeX apenas para os passos de tempo múltiplos de 0.05
#     dados = pd.DataFrame({
...
#         # Aquui será acrescetando a posição da estabilidade com o label: "Posição da Estabilidade" e os dados
#     })
#     dados.to_latex(f"./code/tables/{nomeMetodo}.tex", index=False)
...
for nomeMetodo, limitador in metodos.items():
    ...
    # Obter o último resultado de estabilidade (menor dx possível)
    last_key = list(resultadosEstabilidade.keys())[-1]
    last_result = resultadosEstabilidade[last_key]
    fator = 8  # Correspondente ao menor dx (fator de redução máximo)
    dx_smallest = intervaloEspacial / fator
    passoEstabilidade = int(0.05 / dx_smallest)
    passosTempoEstabilidade = np.arange(0, len(last_result["posicoes"]), passoEstabilidade)
    ...
    dados = pd.DataFrame({
        ...
        f"{nomeMetodo} t=3": resultados[nomeMetodo]["t=3"][passosTempo],
        f"{nomeMetodo} t=5": resultados[nomeMetodo]["t=5"][passosTempo],
        "Posicao da Estabilidade": last_result["posicoes"][passosTempoEstabilidade],
        # "Densidade Estabilidade": last_result["densidadeFinal"][passosTempoEstabilidade]
    })
    dados.to_latex(f"./code/tables/{nomeMetodo}.tex", index=False)
